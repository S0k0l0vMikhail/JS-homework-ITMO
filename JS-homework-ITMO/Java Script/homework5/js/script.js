let goods = {
        piano: {
            id:1,
            title: "Пианино",
            price: 3000,
            count: 25
        },
        guitar: {
          id:2,
            title: "Гитара",
            price: 1200,
            count: 40
        },
        drum: {
          id:3,
            title: "Барабаны",
            price: 2700,
            count: 12
        },
        flute: {
          id:4,
            title: "Флейта",
            price: 900,
            count: 50
        },
        harp: {
          id:5,
            title: "Арфа",
            price: 3400,
            count: 5
        }
    };
    console.log('Задача 1');
    //Написать функцию со следующими аргументами: объект, from (значение от), to (значение до). Функция должна вернуть объект с товарами, цены которых лежат в диапазоне от значения from до значения to. Пример вызова функции getGoods(goods, 2000, 3000); в данном случае функция должна вернуть все товары, у которых цена в диапазоне от 2000 до 3000. Для проверки функции используйте объект goods из файла с урока.

    function verniprice(object, from, to) {
      let arr1 = [];
      for (let keyProd in object){
        if ((object[keyProd].price>= from) && (object[keyProd].price <= to) ){
          arr1.push(object[keyProd]);
        }
      }return arr1;
    }

console.log(verniprice(goods, 10, 900));
